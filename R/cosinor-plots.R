...
ggellipse <- function(object, level = 0.95, ...) {
  ...
  ggplot() +
    ...
    # Axes
    geom_line(
      aes(x = c(0, 0), y = c(-2 * amp1, 2 * amp1)),
      lty = 5,
      col = "grey"
    ) +
    geom_line(
      aes(y = c(0, 0), x = c(-2 * amp1, 2 * amp1)),
      lty = 5,
      col = "grey"
    ) +
...
ggcosinor <- function(object, labels = TRUE, ...) {
  ...
  if ("cosinor" %in% class(object)) {
    ...
    if (p == 1 & object$type == "Individual") {
      ...
      stop(
        "`ggcosinor` does not currently support plotting population cosinor models.",
        call. = FALSE
      )
    }
  ...
  gres <- list(
    geom_segment(
      aes(x = t, xend = t, y = y, yend = .fitted),
      data = aug,
      alpha = 0.3
    ),
    geom_point(
      aes(x = t, y = y, colour = abs(.resid), size = abs(.resid)),
      data = aug
    ),
  ...
  switch(
    ...
    mcomp = {
      gl <- list(
        ...
        geom_text(
          aes(
            x = (bathyphase + orthophase) / 2 + max(aug$t) / 4,
            y = 1.01 * mesor
          ),
...
ggmulticosinor <- function(object, labels, ...) {
  ...
  g <- ggplot() +
    stat_smooth(
      aes(x = t, y = .fitted, colour = .id),
      data = aug,
      method = "gam",
      size = 1.2
    ) +
    geom_hline(
      aes(yintercept = mesor, colour = .id),
      data = features,
      alpha = 0.5
    ) +
    geom_vline(
      aes(xintercept = orthophase, colour = .id),
      data = features,
      alpha = 0.5
    ) +
    geom_vline(
      aes(xintercept = bathyphase, colour = .id),
      data = features,
      alpha = 0.5
    ) +
  ...
  gl <- list(
    ...
    # Peaks
    geom_point(
      aes(x = orthophase, y = peak, colour = .id),
      data = features,
      shape = 19,
      alpha = 0
    ),
    ...
    # Troughs
    geom_point(
      aes(x = bathyphase, y = trough, colour = .id),
      data = features,
      shape = 19,
      alpha = 0
    ),
