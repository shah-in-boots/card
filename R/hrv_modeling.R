...
#'
#' @description `geom_model_error` creates a ggplot geom that can be extended
...
#' @examples
#' ggplot() +
#'   geom_model_error(model) +
#'   labs(x = "HRV values", y = "Depression Score", title = "Model Residuals")
#'
#' @import ggplot2
#'
	...
	switch(type,
				 ...
				 gaussian = {
				 	...
				 	list(
				 		...
				 		geom_segment(
				 			data = model_dx,
				 			aes_string(xend = xaxis, yend = ".fitted"),
				 			alpha = 0.2
				 		),
				 		geom_point(data = model_dx, aes(
				 			colour = abs(.resid), size = abs(.resid)
				 		)),
				 		stat_smooth(
				 			data = model_dx,
				 			method = "lm",
				 ...
				 binomial = {
				 	...
				 	list(
				 		...
				 		theme_minimal(),
				 		ylim(range(as.numeric(
				 			as.character(model_dx[[yaxis]])
				 		)))
				 	)
				 },
				 ...
				 # Catch statement
				 stop(
				 	"Neither a linear (lm) or logistic (glm(family = binomial)) model"
				 ))
}
...
geom_residuals <- function(model) {
	...
	list(
		...
		labs(title = "Residual versus fitted plot",
				 x = "Fitted",
				 y = "Residuals"),
		theme_minimal()
	)
