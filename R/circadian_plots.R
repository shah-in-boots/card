...
#' @param group Grouping variable to apply to the `cvar` (x ~ y)
#' @example
#' # Data
#' df <-
...
#' @details Currently creates a geom_layer that shows a error bar and point estimate of values by group (e.g. clinical status). As t-test values should be available in the data frame, shows points of significance.
#' @return Returns a ggplot object of geom type, other layers can be added on as seen in example.
#' @examples
#' ggplot(df_ttest) + geom_circadian()
#' @export
geom_circadian <- function(time="timegrp", group="catvar") {

	list(
		aes_string(x = time, color = group),
		geom_point(aes(y = mean), position = position_dodge(.3)),
		scale_shape_manual(values = c(1,16)),
		geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.2, position = position_dodge(.3)),
		geom_text(data = df[df$pval < 0.05/24 & df$pval > 0.0005/24, ], aes(y = 2.2, label = "*"), colour = "black", size = 4),
		geom_text(data = df[df$pval < 0.005/24 & df$pval > 0.000005/24, ], aes(y = 2.2, label = "**"), colour = "black", size = 4),
		geom_text(data = df[df$pval < 0.0005/24, ], aes(y = 2.2, label = "***"), colour = "black", size = 4),
